...
/// `b.cond label`.
pub fn b_cond(cond: impl Into<Cond>, pc_offset_bytes: i32) -> u32 {
...
/// `csel wD, wN, wM, cond`.
pub fn csel_w(rd: u8, rn: u8, rm: u8, cond: impl Into<Cond>) -> u32 {
    let cond: Cond = cond.into();
    0x1a80_0000 | (reg5(rm) << 16) | (cond4(cond) << 12) | (reg5(rn) << 5) | reg5(rd)
}

/// `csel xD, xN, xM, cond`.
pub fn csel_x(rd: u8, rn: u8, rm: u8, cond: impl Into<Cond>) -> u32 {
    let cond: Cond = cond.into();
...
/// `cinc wD, wN, cond`.
pub fn cinc_w(rd: u8, rn: u8, cond: impl Into<Cond>) -> u32 {
    let cond: Cond = cond.into();
